        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div id="gc-fork" className="card" style={{ borderLeft: '3px solid #f59e0b' }}>
            ...
            <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>
              ...
              for the knee at every γ.</strong> <em>Scope condition (added 2026-09-15):</em> that holds only if &ldquo;density&rdquo; means a <strong>smoothed</strong> density, and no
              archive document says over what length. Read pointwise, the Sun&apos;s own neighbourhood is the solar wind (&asymp;0.14
              M<sub>&#x2609;</sub>/pc³ at Earth, falling as 1/r²), which crosses every knee in this window between Earth and Saturn; the field
              equation then makes the Sun&apos;s mass inferred from Earth&apos;s orbit and from Saturn&apos;s disagree by order unity (all 240 points
            ...
            <p style={{ color: 'var(--color-text-secondary)', fontSize: '0.9rem' }}>
              <strong>What the verdict column rests on (stated 2026-09-11, visitor researcher persona).</strong>{' '}
              <strong>(i) The cutoffs were set at execution, not at registration.</strong> Session 611 registers a γ and no
              quantitative criterion. &ldquo;Passes&rdquo; = outer-slope mismatch no larger than MOND+EFE&apos;s;
              &ldquo;marginal&rdquo; = under twice it; &ldquo;excluded&rdquo; = more. That benchmark was chosen on 2026-09-07
              because these data admit MOND+EFE. So the table adjudicates a registered <em>γ</em> on a post-hoc{' '}
              <em>scale</em>, which is why the badge says fork, not survives. <strong>(ii) Mass and anisotropy were not refit
              per law.</strong> Every row uses the catalogue mass profile normalized to Baumgardt &amp; Hilker&apos;s Newtonian,
              dark-matter-free N-body masses, with isotropic Jeans. The slope statistic cancels the mass <em>normalization</em>,
              but not the <em>shape</em> of a profile fitted under Newton, so the Newtonian row is the fit baseline and a
              per-law refit could move every modified row, most likely toward better agreement. Anisotropy was varied only as
              a robustness check: radial anisotropy cannot close the density law&apos;s gap at any β &le; 0.8, and the
              tangential anisotropy measured in cluster outskirts makes every row worse. <strong>(iii) What survives
              this:</strong> the ordering (density-keyed at γ = 0.489 is about 2&times; worse than MOND+EFE under the same
              bias) and the shape argument. What does not: reading &ldquo;marginal&rdquo; as a pass. At γ = 2 the density law
          ...
          <div className="card" style={{ borderLeft: '3px solid var(--color-failed)' }}>
            ...
            <p style={{ color: 'var(--color-text-secondary)' }}>
              ...
              S up to 2.67, but only by introducing signaling.</strong> None of the three reaches the
              Tsirelson bound (2&radic;2 &asymp; 2.83) without signaling. <strong>Scope (2026-09-11, visitor researcher
              persona):</strong> that is a statement about three constructions, not about the substrate class. Toner &amp;
              Bacon (PRL 91, 187904, 2003) reproduce the singlet correlations exactly with local hidden variables plus one{' '}
