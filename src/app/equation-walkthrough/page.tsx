...
const steps = [
  ...
  },
  {
    ...
    key: 'ρ_crit is a saturation knee, not a critical density — calling it "transition density" is misleading. Where it sits on the curve is γ-dependent and must always be quoted with its γ: C(ρ_crit) = 0.88 at γ = 2, but 0.33 at the SPARC-fitted γ ≈ 0.49 — near saturation in one case, well below the halfway point in the other.',
  },
  {
  ...
  },
  {
    ...
    explanation: 'tanh (hyperbolic tangent) is a saturation function: over all real inputs it ranges (−1, +1), but since the argument γ·ln(ρ/ρ_crit + 1) is always ≥ 0, C stays in [0, 1) — like a dimmer switch with a soft floor and soft ceiling. Feed it a small number → outputs near 0. Feed it a large number → outputs near 1. No abrupt jump — just a smooth S-curve. Any sigmoid (logistic, erf, Hill) would serve similarly — and in fact the composite tanh∘ln is not merely similar to a Hill function, it IS one exactly (Step 3\'s identity), so tanh is a parametrization choice, not a derived result. The +1 inside the log is a regulator chosen for finiteness at ρ = 0; it is load-bearing — it asymmetrizes the sigmoid, moves the knee off 0.5, and excludes any pure power-law behavior as ρ → 0. How far off 0.5 depends on γ: C(ρ_crit) = tanh(γ·ln 2), which is 0.88 at γ = 2 and 0.33 at the SPARC-fitted γ ≈ 0.49. The γ = 2 value is the one this page used to quote bare; it inherits the pin the RAR refuted at ΔBIC = +184, so it is not the default reading.',
        ...
        <div className="card" style={{ marginBottom: '1.5rem', borderLeft: '3px solid var(--color-accent-violet)' }}>
          ...
          <table style={{ fontSize: '0.85rem', color: 'var(--color-text-secondary)', borderCollapse: 'collapse', width: '100%' }}>
            <tbody>
              ...
              <tr><td style={{ padding: '0.2rem 0.5rem 0.2rem 0', fontFamily: 'monospace', color: 'var(--color-accent-violet)', whiteSpace: 'nowrap' }}>&#x03C1;<sub>crit</sub></td><td>Reference density &mdash; a saturation knee, not a critical point (the &ldquo;crit&rdquo; in the name is historical). <strong>Important:</strong> not the midpoint of C, and <em>where</em> it sits is set by &#x03B3; &mdash; C(&#x03C1;<sub>crit</sub>) = tanh(&#x03B3;&middot;ln&nbsp;2). At &#x03B3;&nbsp;=&nbsp;2 that is 0.88 (near saturation); at the SPARC-fitted &#x03B3;&nbsp;&#x2248;&nbsp;0.49 it is 0.33 (below the half-way point). Always quote the &#x03B3;: a bare &ldquo;0.88&rdquo; silently assumes the pin the RAR refuted at &#x0394;BIC&nbsp;=&nbsp;+184.</td></tr>
              <tr><td style={{ padding: '0.2rem 0.5rem 0.2rem 0', fontFamily: 'monospace', color: 'var(--color-accent-violet)', whiteSpace: 'nowrap' }}>&#x03B3;</td><td>Controls sigmoid sharpness. The original proposal was &#x03B3; = 2/&#x221A;N<sub>corr</sub>, with N<sub>corr</sub> = number of particles moving together, so one particle would give &#x03B3;=2 (sharp) and a million &#x03B3;=2&times;10&#x207B;&#x00B3; (flat). That proposal failed: its sign is inverted against real collective physics (audited-negative; see the red box below). Where &#x03B3; is used today it is fitted, not computed from N<sub>corr</sub>.</td></tr>
