...
const tests: Test[] = [
  ...
  },
  {
    ...
    cost: '$0',
    time: 'EXECUTED (2026-07-14)',
    prediction: 'Computed from the framework\'s own formula (C(a) = Ω_m + (1−Ω_m)x/(1+x), boost capped at 1/Ω_m = 3.17): BTFR slope n = 3.35 ± 0.07. [Which acceleration, stated 2026-09-14: the TEST-09 and TEST-10 scripts evaluate C explicitly at the baryonic acceleration, with x = (g_bar/a₀)^(1/φ), a₀ = 1.05×10⁻¹⁰ m/s², and g_obs = g_bar/C(g_bar). There is no implicit solve in g_obs. The RAR-shape fits on /galaxy-rotation are the ones solved implicitly in g_obs. The 3.17 cap holds under either keying, because C ≥ Ω_m for every x ≥ 0.] The previously listed "deep-MOND sample → n ≈ 4" limb is retracted — a bounded boost has NO deep-MOND regime. As g_bar → 0 the boost saturates at 3.17, a constant rescaling of G, which is Newtonian: the framework\'s deep limit is Keplerian, with rotation curves that fall as 1/√r where MOND\'s stay flat. MOND\'s n = 4 comes precisely from the √(a₀/g_bar) divergence a bounded boost cannot follow. [Precision, 2026-09-25, visitor graduate-physics persona: the older wording here, "deep limit n → 2 (verified numerically: 2.01)", holds only for a sample measured at one common radius. A Keplerian law has no radius-independent BTFR slope: for measurement radii R ∝ M^α it gives n = 2/(1−α), so n ≈ 3 for real disc radii (α ≈ 0.3–0.4), and n = 4 if each galaxy is read where g_bar falls to a fixed value (α = ½). What is radius-independent is the declining curve shape. The verdict does not move: the 3.35 above was computed at each SPARC galaxy\'s real measured radii.]',
...
export default function Tier1Existing() {
  return (
    <>
      ...
      <ValidationBadge status="failed" label={`Failed — all ${EXECUTED_COUNT} numbered executed tests (+ the unnumbered RAR-shape test, card under Recommended Start): ${TALLY.killsFired.length} with the kill fired (${TALLY.killsFired.join('; ')}) + ${TALLY.underpowered.length} disfavored but underpowered as registered, not counted as a refutation (${TALLY.underpowered.join('; ')}). Convention-dependent: ${TALLY.conventionDependent.join(' and ')} fire at B_max = 1/Ω_m but not at every candidate ceiling (TEST-09 does not survive the 2026-09-18 sweep; convention-free, B_max ≲ 5.4 is excluded by the BTFR slope and B_max ≲ 14 by SPARC dwarfs); the ceiling both tests share is excluded at every Ω_m-based convention by the weak-lensing RAR (Brouwer+2021; see /honest-assessment#refutation-ledger); whether they stay in the count of 6 is pending the project lead (dp). Not executed: TEST-04 withdrawn; TEST-02 self-eliminating; rest untested. Site-ledger split: ${ROOTS_SENTENCE}. ${SIX_TRACE}. $0 cost`} />
