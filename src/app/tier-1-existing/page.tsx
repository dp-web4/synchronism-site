...
const tests: Test[] = [
  ...
  },
  {
    ...
    alert: 'SIGN CORRECTION (2026-06-06): The prediction direction was reversed on this page from 2026-05-05 to 2026-06-06. The correct direction is lower density → stronger deviation (g_eff = g_N/C(ρ) gives low ρ → low C → larger boost). Both MOND+EFE and C(ρ) agree on this direction: denser environments strengthen the external field, suppressing the low-acceleration anomaly (MOND EFE) and raising C toward Newtonian (Synchronism). Substrate contingency (2026-05-17): this test is conditional on the wide-binary anomaly existing in the first place. As of 2026, the anomaly\'s existence is actively disputed. Chae (2023) detected the anomaly in Gaia DR3; Banik et al. (2024) and Pittordis & Sutherland (2023) found no significant anomaly under improved quality cuts and proper 3D velocity priors on the same DR3 data; Saurabh et al. (2024) also finds no detection with stricter contamination filters. EFE direction (2026-06-05 explorer): after the sign correction, C(ρ) and MOND+EFE predict the SAME direction (lower-density → stronger deviation) — so the direction is not a discriminator. The amplitude IS discriminating: C(ρ)-density form predicts ~0.05–0.4% velocity deviation in the clean sample (Newtonian null level) vs MOND ~18%; Gaia DR3 reach is ~80× insufficient to see either signal at derived ρ_crit. TEST-02 cannot confirm C(ρ) over Newton (both predict near-null); a real anomaly à la Chae would refute C(ρ)-density form. This is the third C(a)→C(ρ) fork: the acceleration form C(a) predicts the anomaly amplitude correctly but is MOND-degenerate (and refuted by SPARC RAR at ΔBIC=+184); the density form C(ρ) is structurally distinct from MOND but predicts the Newtonian null.',
  },
  {
    id: 'TEST-03',
